...
def save_curves(losses, accs, outdir):
    plt.figure()
    plt.plot(losses, label="train_loss", marker='o')
    plt.plot(accs, label="val_pair_acc", marker='o')
    plt.xlabel("epoch")
    plt.title("Siamese Training")
    plt.legend()
    plt.tight_layout()
...
def train_siamese(args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_acc:
            ...
            torch.save({"state_dict": ema_model.state_dict(), "args": vars(args)},
                       os.path.join(args.outdir, f"siamese_epoch{epoch+1}.pt"))
    ...
    if args.final_eval:
        ...
        ckpt_path = os.path.join(args.outdir, f"siamese_epoch{args.epochs}.pt")
        print(f"[Final Eval] Using checkpoint: {ckpt_path}")
        evaluate_linear_probe(ckpt_path, args.csv, args.images, args.img_size)
